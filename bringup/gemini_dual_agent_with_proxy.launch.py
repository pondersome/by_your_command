...
def generate_launch_description():
    ...
    # Use proxy-specific configs that connect to port 8766
    conv_agent_config = os.path.join(pkg_dir, 'config', 'gemini_conversational_agent_proxy.yaml')
    cmd_agent_config = os.path.join(pkg_dir, 'config', 'gemini_command_agent_proxy.yaml')
    ...
    # Must start before bridge to be ready for agent connections
    ...
    startup_message = LogInfo(
        msg=[
            '🚀 Starting Gemini Dual Agent System with Reflection Proxy\n',
            '🔄 Reflection Proxy: Enabling cross-agent communication\n',
    ...
    # Agents and proxy run as separate processes
    agents_group = GroupAction([
        reflection_proxy,  # Added reflection proxy
